...
class FindPath(FiberTest):
    ...
    def test_balala(self):
        ...
        for j in range(1000):
            for i in range(len(self.fibers)):
                try:
                    payment_hash = self.send_payment(
                        self.fibers[i], self.fibers[i], 1 * 10000000, False, None, 0
                    )
                    hashes[i].append(payment_hash)
                except:
                    pass
    ...
    def test_00002233(self):
        ...
        #     #     "peer_id":self.fibers[i+1].get_peer_id()
        #     # })
        #     self.fibers[i].connect_peer(self.fibers[i+1])
        # for i in range(2):
        #     # self.fibers[i + 3].get_client().disconnect_peer({
        #     #     "peer_id": self.fibers[i + 4].get_peer_id()
        #     # })
        #     self.fibers[i+3].connect_peer(self.fibers[i + 4])
        # for i in range(3):
        #     # self.fibers[i ].get_client().disconnect_peer({
        #     #     "peer_id": self.fibers[i + 3].get_peer_id()
        #     # })
        #     self.fibers[i ].connect_peer(self.fibers[i + 3])
