...
class LndNode:
    ...
    def __init__(self,tmp_path,listen_port,rpc_port,rest_port):
        ...
        self.lnd = f"{get_project_root()}/download/lnd/lnd-darwin-arm64-v0.19.3-beta/lnd"
        self.lnd_cli = f"{get_project_root()}/download/lnd/lnd-darwin-arm64-v0.19.3-beta/lncli"
        """
    ...
    def start(self):
        run_command(f'{self.lnd} --lnddir="{self.tmp_path}" &>/dev/null & echo $! > {self.tmp_path}/lnd.pid')
        retries=30
    ...
    def ln_cli_with_cmd_without_json(self,cmd):
        return run_command(f'{self.lnd_cli} -n regtest --lnddir="{self.tmp_path}" --no-macaroons --rpcserver "localhost:{self.rpc_port}" {cmd}')

    def ln_cli_with_cmd(self,cmd):
        return json.loads(run_command(f'{self.lnd_cli} -n regtest --lnddir="{self.tmp_path}" --no-macaroons --rpcserver "localhost:{self.rpc_port}" {cmd}'))
    ...
    def open_channel(self,peerNode,local_amt,sat_per_vbyte,min_confs):
        ...
        self.ln_cli_with_cmd(f'openchannel --node_key {node_key} --connect {peer_address} --local_amt {local_amt} --sat_per_vbyte {sat_per_vbyte} --min_confs {min_confs}')
        ##   echo "openchannel"
        #   local retries=5
