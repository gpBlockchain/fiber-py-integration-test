...
class BtcNode:
    ...
    def prepare(self, other_config={}):
        ...
        shutil.copy(
            "{root_path}/source/lnd-init/bitcoind/bitcoin.conf".format(root_path=get_project_root()),
            self.tmp_path,
        )


    def start(self):
        run_command(f'{self.bin} -conf="{self.config}" -datadir="{self.tmp_path}" -daemonwait -pid="{self.tmp_path}/bitcoind.pid"')
        #   bitcoin-cli -conf="$bitcoind_conf" -datadir="$bitcoind_dir" -rpcwait createwallet dev >/dev/null
        run_command(f'{self.cli} -conf="{self.config}" -datadir="{self.tmp_path}" -rpcwait createwallet dev >/dev/null')
        print("bitcoind wallet created")
        #   bitcoin-cli -conf="$bitcoind_conf" -generate 101 >/dev/null
        self.miner(101)

    def miner(self,number):
        run_command(f'{self.cli} -conf="{self.config}" -datadir="{self.tmp_path}" -generate {number} >/dev/null')
    ...
    def sendtoaddress(self,address,amount,fee_rate):
        return run_command(f'{self.cli} -conf="{self.config}" -datadir="{self.tmp_path}" -named sendtoaddress address="{address}" amount={amount} fee_rate={fee_rate}')